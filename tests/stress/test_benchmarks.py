...
class TestAsyncVsSyncBenchmarks:
    ...
    async def test_async_vs_sync_creation(self, tmp_path: Path):
        ...
        assert speedup > 0.5 or async_time < 5.0, f"Async performance acceptable: {speedup:.2f}x, {async_time:.2f}s"
    ...
    async def test_concurrent_queries_speedup(self, tmp_path: Path):
        ...
        await asyncio.gather(*[
            kernel.retrieve_nodes_async(q, top_k=5)
            for q in queries
        ])
...
class TestScalabilityBenchmarks:
    ...
    async def test_query_time_scaling(self, tmp_path: Path):
        ...
        if query_times[0] > 0:
            ...
            logger.info(f"Scaling factor: {scaling_factor:.2f}x for {size_factor:.0f}x size increase")
