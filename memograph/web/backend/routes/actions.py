...
async def get_actions(
    ...
    actions = action_logger.get_recent_actions(
        limit=limit, action_type=action_type, memory_id=memory_id
    )

    if grouped:
        actions = action_logger.group_consecutive_actions(actions)
        return {"grouped": True, "groups": actions, "total_groups": len(actions)}

    return {"grouped": False, "actions": actions, "total": len(actions)}
...
async def get_memory_history(memory_id: str) -> dict[str, Any]:
    ...
    return {"memory_id": memory_id, "actions": history, "total": len(history)}
