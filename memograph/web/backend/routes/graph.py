...
async def get_graph_data(
    request: Request,
    limit: Optional[int] = Query(None, ge=1, le=500, description="Maximum number of nodes to return"),
    ...
    try:
        logger.debug(f"Graph request: limit={limit}, min_salience={min_salience}, tags={tags}, focus_node={focus_node}")
        ...
        if focus_node:
            ...
            logger.debug(f"After focus_node filter (node={focus_node}, depth=2): {len(filtered_nodes)} nodes")
