...
def check_server_json() -> bool:
    ...
    if missing_fields:
        print_status("Required fields present", False, f"Missing: {', '.join(missing_fields)}")
        return False
    ...
    if actual_namespace != expected_namespace:
        print_status("Correct namespace", False, f"Expected: {expected_namespace}, Got: {actual_namespace}")
        return False
    ...
    if actual_command != expected_command:
        print_status("Execution command", False, f"Expected: {expected_command}, Got: {actual_command}")
        return False
    ...
    if actual_args != expected_args:
        print_status("Execution args", False, f"Expected: {expected_args}, Got: {actual_args}")
        return False
...
def check_mcp_server() -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print_status("MCP server executable", False, f"Exit code: {result.returncode}")
            if result.stderr:
...
def check_github_repo() -> bool:
    ...
    try:
        ...
        if is_public:
            ...
            print_status("Repository accessible", True, "https://github.com/Indhar01/MemoGraph")
            return True
        else:
